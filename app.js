...
function activatePredTarget(id){if(!id) return; if(id.startsWith('ps_')) setActivePredSave(id.slice(3)); else setActivePredInput(id);}
...
function rankHtml(r,title,sub,showChampion=false,phase='groups'){return `<div class="card"><h2>${title}</h2><p class="muted">${sub}</p>${r.map((x,i)=>{let avg=x.played?((x.pts/x.played).toFixed(1)):0; const m=playerMetrics(x.p.id,phase); const mood=rankMoodEmoji(i+1,r.length,x.p.id,phase); const medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':''; const nonExact=rankHitList(x.p.id,phase,false).length; const rid=`${phase||'all'}_${x.p.id}`.replace(/[^a-zA-Z0-9_-]/g,'_'); return `<div class="rank-line ${i===0?'top1':''}"><div class="rank-points"><span class="rank-big-emoji">${mood}</span><span class="rank-score">${x.pts}</span><small>PTS</small></div><div><div class="rank-pos"><span class="rank-fun">${medal} ${i+1}.</span> ${esc(x.p.name)} <span class="pill">${moveBadge(x.p.id,i+1,phase)}</span></div><div class="rank-meta">${x.exact} exacto(s) · ${nonExact} acierto(s) no exactos · ${m.effectiveness}% efectividad · 🔥 racha ${m.currentStreak} / mejor ${m.bestStreak} · Promedio ${avg}${showChampion?` · Campeón +10: ${x.champBonus?'✅':'—'}`:''}</div></div><div class="rank-actions">${showChampion?`<div class="pill rank-exact-pill">👑 +10 ${x.champBonus?'✅':'—'}</div>`:''}<button type="button" class="pill rank-toggle-pill rank-exact-pill" onclick="toggleRankHits('${rid}_exact',this)">🎯 ${x.exact}</button><button type="button" class="pill rank-toggle-pill rank-exact-pill" onclick="toggleRankHits('${rid}_hits',this)">✅ ${nonExact}</button></div></div><div id="rank_${rid}_exact" class="rank-detail-panel">${rankHitPanelHtml(x.p.id,phase,true)}</div><div id="rank_${rid}_hits" class="rank-detail-panel">${rankHitPanelHtml(x.p.id,phase,false)}</div>`}).join('')||'<p>Sin participantes.</p>'}</div>`}
function rankHitList(pid,phase=null,exactOnly=true){const list=(phase?matches.filter(m=>m.phase===phase):matches).slice().sort((a,b)=>new Date(a.date)-new Date(b.date)); return list.map(m=>{const pr=predFor(pid,m.id), r=resFor(m.id); const got=pointsFor(pr,m); if(!r||r.home_score===null||r.away_score===null||!pr||pr.home_score===null||pr.away_score===null) return null; const isExact=(+pr.home_score===+r.home_score&&+pr.away_score===+r.away_score); if(exactOnly?!isExact:!(got>=1&&!isExact)) return null; return {m,pr,r,got};}).filter(Boolean);}
function rankHitPanelHtml(pid,phase=null,exactOnly=true){const rows=rankHitList(pid,phase,exactOnly); const title=exactOnly?'🎯 Marcadores exactos':'✅ Aciertos no exactos'; if(!rows.length) return `<div class="rank-detail-head">${title}</div><p class="muted">Todavía no hay partidos en esta lista.</p>`; return `<div class="rank-detail-head">${title} · ${rows.length}</div>${rows.map(x=>`<div class="rank-detail-row"><div><b>${teamLabel(x.m.home)} ${x.r.home_score}-${x.r.away_score} ${teamLabel(x.m.away)}</b><div class="small">Pronóstico: ${x.pr.home_score}-${x.pr.away_score} · ${esc(x.m.group||x.m.round||'')}</div></div><small>${fmtDate(x.m.date)} · +${x.got} pts</small></div>`).join('')}`;}
function toggleRankHits(id,btn){const el=document.getElementById('rank_'+id); if(!el) return; const open=el.classList.toggle('open'); if(btn) btn.classList.toggle('open',open);}
...
function retameTargetsOptions(){return groupMembers().filter(p=>session&&p.id!==session.id).map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join('')}
function retameScopeHtml(prefix, directLabel='Retar directamente a'){
  return `<div class="retame-scope-box"><label>Tipo de reto</label><div class="scope-row"><label class="scope-option"><input type="radio" name="${prefix}Scope" value="open" checked onchange="toggleRetameDirect('${prefix}')"> 🌎 Abierto</label><label class="scope-option"><input type="radio" name="${prefix}Scope" value="direct" onchange="toggleRetameDirect('${prefix}')"> 🎯 Directo</label><select id="${prefix}Target" class="hidden"><option value="">${directLabel}</option>${retameTargetsOptions()}</select></div><div class="small">Abierto: cualquiera puede aceptar. Directo: solo el participante elegido puede aceptar.</div></div>`;
}
function toggleRetameDirect(prefix){const val=document.querySelector(`input[name="${prefix}Scope"]:checked`)?.value||'open'; const sel=document.getElementById(prefix+'Target'); if(sel) sel.classList.toggle('hidden',val!=='direct');}
function retameScopeValue(prefix){const type=document.querySelector(`input[name="${prefix}Scope"]:checked`)?.value||'open'; const tid=document.getElementById(prefix+'Target')?.value||''; const p=groupMembers().find(x=>x.id===tid); return {accessType:type,targetId:type==='direct'?tid:'',targetName:type==='direct'?(p?.name||''):''};}
function retameVisibleToMe(x){if(!session) return false; if((x.accessType||'open')!=='direct') return true; return x.creatorId===session.id || x.targetId===session.id || (x.players||[]).some(p=>p.pid===session.id) || (x.answers||[]).some(a=>a.pid===session.id) || (x.requests||[]).some(r=>r.pid===session.id) || (x.accepted||[]).some(a=>a.pid===session.id);}
function retameScopeBadge(x){return (x.accessType||'open')==='direct'?`<span class="retame-direct-badge">🎯 Directo a ${esc(x.targetName||'participante')}</span>`:`<span class="retame-open-badge">🌎 Abierto</span>`;}
async function createRetameChallenge(){try{const m=matches.find(x=>x.id===retameMatch.value); if(!m) return alert('Selecciona un partido.'); if(new Date(m.date).getTime()<Date.now()) return alert('Este partido ya inició o terminó.'); const scope=retameScopeValue('retame'); if(scope.accessType==='direct'&&!scope.targetId) return alert('Elige a quién vas a retar directamente.'); const item={id:'r_'+Date.now()+'_'+Math.random().toString(36).slice(2),groupId:currentGroupId(),creatorId:session.id,creatorName:session.name,matchId:m.id,creatorPick:retamePick.value,rewardType:retameRewardType.value,reward:retameReward.value.trim(),message:retameMsg.value.trim(),accessType:scope.accessType,targetId:scope.targetId,targetName:scope.targetName,status:'open',requests:[],accepted:[],rejected:[],createdAt:new Date().toISOString()}; await saveRetameList([item,...retameList()].slice(0,250)); retameReward.value=''; retameMsg.value=''; retameCreateMsg.className='status ok'; retameCreateMsg.textContent=scope.accessType==='direct'?'Reto directo publicado. Solo esa persona puede aceptarlo.':'Reto abierto publicado. Cualquiera puede aceptarlo.'; notifyGroup('retame',scope.accessType==='direct'?'🎯 Nuevo reto directo':'🔥 Nuevo reto abierto',`${session.name}: ${m.home} vs ${m.away}`).catch(()=>{}); renderRetame();}catch(e){retameCreateMsg.className='status err'; retameCreateMsg.textContent='Error: '+e.message}}
async function requestRetame(id,pick){const list=retameList(); const c=list.find(x=>x.id===id); if(!c||!['open','pending_creator'].includes(c.status)) return alert('Este reto ya no está abierto.'); if(c.creatorId===session.id) return alert('No puedes aceptar tu propio reto.'); if((c.accessType||'open')==='direct'&&c.targetId!==session.id) return alert('Este reto directo es para otro participante.'); if(pick===c.creatorPick) return alert('Debes elegir una opción diferente a la del creador.'); const req={pid:session.id,name:session.name,pick,at:new Date().toISOString()}; const updated=list.map(x=>x.id===id?{...x,requests:[...(x.requests||[]).filter(r=>r.pid!==session.id),req],status:'pending_creator'}:x); await saveRetameList(updated); renderRetame();}
...
async function createTriviaRoom(){const scope=retameScopeValue('trivia'); if(scope.accessType==='direct'&&!scope.targetId){triviaCreateMsg.textContent='Elige a quién vas a retar directamente.'; return;} const invited=[...document.querySelectorAll('.trivia-invite:checked')].map(x=>x.value); const members=groupMembers(); let ids=[session.id]; if(scope.accessType==='direct') ids.push(scope.targetId); ids=[...new Set([...ids,...invited])]; if(scope.accessType==='open'&&ids.length<1) ids=[session.id]; const players=ids.map(pid=>{const p=members.find(x=>x.id===pid)||{id:pid,name:pid}; return {pid:p.id,name:p.name,ready:p.id===session.id,score:0,correctFirst:0};}); const category=document.getElementById('triviaCategory')?.value||'aleatoria'; const item={id:'tr_'+Date.now()+'_'+Math.random().toString(36).slice(2),type:'trivia_room',creatorId:session.id,creatorName:session.name,accessType:scope.accessType,targetId:scope.targetId,targetName:scope.targetName,category,seconds:+(document.getElementById('triviaSeconds')?.value||15),status:'waiting',players,questions:triviaPool(category),current:0,answers:{},winners:[],createdAt:new Date().toISOString()}; await saveRetameGames([item,...retameGamesList()].slice(0,200)); document.getElementById('triviaCreateBox')?.classList.add('hidden'); notifyGroup('retame',scope.accessType==='direct'?'🎯 Trivia directa':'⚡ Trivia abierta',`${session.name} creó una sala de trivia`).catch(()=>{}); renderRetame();}
async function joinTriviaRoom(id){const list=retameGamesList(); const updated=list.map(g=>{if(g.id!==id||g.type!=='trivia_room'||g.status!=='waiting') return g; if((g.accessType||'open')==='direct'&&g.targetId!==session.id) return g; if((g.players||[]).some(p=>p.pid===session.id)||g.creatorId===session.id) return g; return {...g,players:[...(g.players||[]),{pid:session.id,name:session.name,ready:false,score:0,correctFirst:0}]};}); await saveRetameGames(updated); renderRetame();}
...
function triviaRoomCard(g){
  ...
  let body='';
  if(g.status==='waiting'){
    const canJoinRoom=!mine&&!isCreator&&g.status==='waiting'&&((g.accessType||'open')!=='direct'||g.targetId===session.id); const joinBtn=canJoinRoom?`<button class="primary" onclick="joinTriviaRoom('${g.id}')">Aceptar / entrar al reto</button>`:''; const readyBtn=(mine&&!mine.ready)?`<button class="good" onclick="triviaSetReady('${g.id}')">Estoy listo ✅</button>`:'';
    ...
    body=`<div class="trivia-progress">Sala de espera · ${readyCount}/${(g.players||[]).length} listos · ${g.questions?.length||10} preguntas</div><div class="players">${players}</div>${joinBtn}${readyBtn}${creatorBtns}`;
  ...
  return `<div class="trivia-room"><div class="retame-head"><div><div class="retame-title">⚡ Trivia Multijugador</div><div class="meta">Creada por ${esc(g.creatorName)} · ${fmtDateTime(g.createdAt)}</div></div><span class="retame-status">${g.status==='waiting'?'Sala de espera':g.status==='playing'?'En juego':'Finalizada'}</span></div>${retameScopeBadge(g)}${body}</div>`;
}
...
async function createExpressChallenge(){const m=matches.find(x=>x.id===expressMatch.value); if(!m) return alert('Selecciona un partido.'); const scope=retameScopeValue('express'); if(scope.accessType==='direct'&&!scope.targetId) return alert('Elige a quién vas a retar directamente.'); const item={id:'g_'+Date.now()+'_'+Math.random().toString(36).slice(2),type:'express',creatorId:session.id,creatorName:session.name,accessType:scope.accessType,targetId:scope.targetId,targetName:scope.targetName,matchId:m.id,question:expressQuestion.value,status:'open',answers:[],createdAt:new Date().toISOString()}; await saveRetameGames([item,...retameGamesList()].slice(0,200)); notifyGroup('retame',scope.accessType==='direct'?'🎯 Reto Express directo':'⚽ Reto Express abierto',`${m.home} vs ${m.away}`).catch(()=>{}); renderRetame();}
async function createGroupChallenge(){const m=matches.find(x=>x.id===groupRetameMatch.value); if(!m) return alert('Selecciona un partido.'); const scope=retameScopeValue('group'); if(scope.accessType==='direct'&&!scope.targetId) return alert('Elige a quién vas a retar directamente.'); const item={id:'g_'+Date.now()+'_'+Math.random().toString(36).slice(2),type:'grupal',creatorId:session.id,creatorName:session.name,accessType:scope.accessType,targetId:scope.targetId,targetName:scope.targetName,matchId:m.id,question:(groupRetameQuestion.value||'¿Quién gana el partido?').trim(),status:'open',answers:[],createdAt:new Date().toISOString()}; await saveRetameGames([item,...retameGamesList()].slice(0,200)); notifyGroup('retame',scope.accessType==='direct'?'🎯 Reto grupal directo':'👥 Reto grupal abierto',`${m.home} vs ${m.away}`).catch(()=>{}); renderRetame();}
async function answerRetameGame(id,answer){const list=retameGamesList(); const updated=list.map(g=>{if(g.id!==id||g.status!=='open') return g; if((g.accessType||'open')==='direct'&&g.targetId!==session.id&&g.creatorId!==session.id) return g; if((g.answers||[]).some(a=>a.pid===session.id)) return g; const ans={pid:session.id,name:session.name,answer,at:new Date().toISOString()}; const answers=[...(g.answers||[]),ans]; if(g.type==='trivia' && String(answer)===String(g.correctAnswer)){return {...g,answers:[...answers.slice(0,-1),{...ans,correct:true}],status:'closed',winnerId:session.id,winnerName:session.name,closedAt:new Date().toISOString(),winningAnswer:answer};} return {...g,answers};}); await saveRetameGames(updated); renderRetame();}
...
function retameGameCard(g){const m=matches.find(x=>x.id===g.matchId)||{}; const opts=retameGameOptions(g); const mine=(g.answers||[]).find(a=>a.pid===session.id); const isCreator=g.creatorId===session.id; const title={trivia:'⚡ Trivia Flash',express:'⚽ Reto Express',grupal:'👥 Reto Grupal'}[g.type]||'🔥 Juego'; const match=m.home?` · ${esc(m.home)} vs ${esc(m.away)}`:''; const canAnswer=g.status==='open'&&!mine&&g.creatorId!==session.id&&((g.accessType||'open')!=='direct'||g.targetId===session.id); const buttons=canAnswer?`<div class="retame-game-options">${opts.map(o=>`<button onclick="answerRetameGame('${g.id}','${esc(o)}')">${esc(o)}</button>`).join('')}</div>`:(mine?`<span class="retame-chip">Tu respuesta: ${esc(mine.answer)}</span>`:''); const close=(isCreator&&g.status==='open'&&g.type!=='trivia')?`<div class="retame-game-options"><b>Cerrar con:</b>${opts.map(o=>`<button class="good" onclick="closeRetameGame('${g.id}','${esc(o)}')">${esc(o)}</button>`).join('')}<button class="danger" onclick="cancelRetameGame('${g.id}')">Cancelar</button></div>`:(isCreator&&g.status==='open'?`<button class="danger" onclick="cancelRetameGame('${g.id}')">Cancelar</button>`:''); const rows=(g.answers||[]).map(a=>`<div class="retame-answer-row"><span>${esc(a.name)}: <b>${esc(a.answer)}</b></span><span>${g.type==='trivia'?(a.correct?'<span class="retame-correct">Correcto</span>':''):(g.status==='closed'&&String(a.answer)===String(g.winningAnswer)?'<span class="retame-correct">Ganó</span>':'')}</span></div>`).join(''); const result=g.status==='closed'?`<div class="status ok">Ganador${(g.winners||[]).length>1?'es':''}: ${g.winnerName?esc(g.winnerName):((g.winners||[]).length?(g.winners||[]).map(pid=>esc(groupMembers().find(p=>p.id===pid)?.name||pid)).join(', '):'Sin ganador')}</div>`:''; return `<div class="retame-game-card"><h3>${title}</h3><div class="meta">${esc(g.creatorName)}${match} · ${fmtDateTime(g.createdAt)} ${g.status==='open'?'<span class="retame-live-pill">Abierto</span>':'<span class="retame-status">Cerrado</span>'}</div>${retameScopeBadge(g)}<p><b>${esc(g.question)}</b></p>${buttons}${close}${result}<div style="margin-top:10px">${rows||'<p class="muted">Aún no hay respuestas.</p>'}</div></div>`;}
...
function updateNavBadges(){if(!session)return; const latest=latestChatActivity(); const mention=chatMentionedMe(latest); setTabBadge('chat',hasUnseen('chat',latest),mention?'@':''); setTabBadge('todos',hasUnseen('todos',latestPredActivity()),''); setTabBadge('pred',pendingTodayPredictions().length>0,''); const retos=retameList().filter(c=>retameVisibleToMe(c)&&['open','pending_creator'].includes(c.status)&&c.creatorId!==session.id&&!((c.requests||[]).some(r=>r.pid===session.id)||(c.accepted||[]).some(a=>a.pid===session.id))); const juegos=retameGamesList().filter(g=>retameVisibleToMe(g)&&['open','waiting'].includes(g.status)&&g.creatorId!==session.id&&!((g.answers||[]).some(a=>a.pid===session.id))&&!((g.players||[]).some(p=>p.pid===session.id&&p.ready)));  const nRetos=retos.length+juegos.length; setTabBadge('retame',nRetos>0,nRetos?String(Math.min(9,nRetos)):'')}
